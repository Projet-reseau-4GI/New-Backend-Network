...
public class MetricsService {
     ...
     *
     * @param platformId optional – filter to a single platform
    ...
    public Mono<DashboardStatsDto> getDashboardStats(
            Long platformId, LocalDateTime from, LocalDateTime to, String period) {
        ...
        return db.sql(sql)
            ...
            .defaultIfEmpty(DashboardStatsDto.builder()
                .totalVerifications(0L).successCount(0L).failureCount(0L)
                .avgProcessingTimeMs(0.0).totalApiTokensCreated(0L).period(period)
                .build());
    }
    ...
    public Flux<TimeSeriesPoint> getVerificationsOverTime(
            Long platformId, LocalDateTime from, LocalDateTime to, String granularity) {
        ...
        String sql =
            "SELECT " +
            "  TO_CHAR(DATE_TRUNC('" + trunc + "', vl.date), '" + dateFormat + "')     AS label, " +
            "  COUNT(*)                                                                 AS total, " +
            "  SUM(CASE WHEN vl.status = 'ACCEPTED' THEN 1 ELSE 0 END)                AS success, " +
            "  SUM(CASE WHEN vl.status = 'REJECTED' THEN 1 ELSE 0 END)                AS failure " +
            "FROM verification_logs vl " +
    ...
    public Flux<HourlyTrafficDto> getHourlyTraffic(
            Long platformId, LocalDateTime from, LocalDateTime to) {

        String whereBase = "WHERE 1=1" + platformFilter(platformId) + dateFilter(from, to);
        String sql =
            "SELECT EXTRACT(HOUR FROM vl.date)::INTEGER AS hour, COUNT(*) AS cnt " +
            "FROM verification_logs vl " +
            whereBase +
    ...
    public Flux<RecentVerificationDto> getRecentVerifications(Long platformId) {
        String whereClause = platformId != null
            ? "WHERE vl.platform_id = " + platformId
            : "WHERE 1=1";

        String sql =
            "SELECT vl.id, vl.platform_id, p.name AS platform_name, vl.date, " +
    ...
    public Flux<UsageStatisticDto> getOverallUsageStatistics() {
        String sql =
            "SELECT platform_id, doc_type, COUNT(*) AS total_logs, " +
