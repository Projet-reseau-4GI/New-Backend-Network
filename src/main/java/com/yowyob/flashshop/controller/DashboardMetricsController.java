...
import com.yowyob.flashshop.service.DashboardExportService;
import com.yowyob.flashshop.service.MetricsService;
...
public class DashboardMetricsController {
        ...
        public Mono<DashboardStatsDto> getStats(
                ...
                Long effectiveId = resolveEffectivePlatformId(auth, platformId);
                LocalDateTime[] range = resolvePeriod(period, from, to);
                return metricsService.getDashboardStats(effectiveId, range[0], range[1], period);
        }
        ...
        public Flux<TimeSeriesPoint> getVerificationsOverTime(
                ...
                Long effectiveId = resolveEffectivePlatformId(auth, platformId);
                LocalDateTime[] range = resolvePeriod(period, from, to);
                return metricsService.getVerificationsOverTime(effectiveId, range[0], range[1], granularity);
        }
        ...
        public Flux<HourlyTrafficDto> getHourlyTraffic(
                ...
                Long effectiveId = resolveEffectivePlatformId(auth, platformId);
                LocalDateTime[] range = resolvePeriod(period, from, to);
                return metricsService.getHourlyTraffic(effectiveId, range[0], range[1]);
        }
        ...
        public Flux<StatusDistributionDto> getStatusDistribution(
                ...
                Long effectiveId = resolveEffectivePlatformId(auth, platformId);
                LocalDateTime[] range = resolvePeriod(period, from, to);
                return metricsService.getStatusDistribution(effectiveId, range[0], range[1]);
        }
        ...
        public Flux<DocTypeBreakdownDto> getDocTypeBreakdown(
                ...
                Long effectiveId = resolveEffectivePlatformId(auth, platformId);
                LocalDateTime[] range = resolvePeriod(period, from, to);
                return metricsService.getDocTypeBreakdown(effectiveId, range[0], range[1]);
        }
        ...
        public Flux<RecentVerificationDto> getRecentVerifications(
                        ...
                        Authentication auth) {
                Long effectiveId = resolveEffectivePlatformId(auth, platformId);
                return metricsService.getRecentVerifications(effectiveId);
        }
        ...
        public Flux<DetailedVerificationDto> getSuccessfulVerifications(
                ...
                Long effectiveId = resolveEffectivePlatformId(auth, platformId);
                LocalDateTime[] range = resolvePeriod(period, from, to);
                return metricsService.getSuccessfulVerifications(effectiveId, range[0], range[1]);
        }
        ...
        public Mono<ResponseEntity<ByteArrayResource>> exportCsv(
                ...
                Long effectiveId = resolveEffectivePlatformId(auth, platformId);
                LocalDateTime[] range = resolvePeriod(period, from, to);
                String filename = "verifid-rapport-"
                                + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd-HHmm"))
                                + ".csv";

                return exportService.exportCsv(effectiveId, range[0], range[1])
        ...
        public Mono<ResponseEntity<ByteArrayResource>> exportPdf(
                ...
                return exportService.exportPdf(effectiveId, range[0], range[1], period)
                                .map(bytes -> ResponseEntity.ok()
                                                .header(HttpHeaders.CONTENT_DISPOSITION,
                                                                ContentDisposition.attachment().filename(filename)
                                                                                .build().toString())
        ...
        private Long resolveEffectivePlatformId(Authentication auth, Long requestedId) {
                if (auth == null)
                        return requestedId;
                ...
                if (isSuperAdmin) {
                        return requestedId; // SuperAdmin can see everything or any specific platform
                }

                // For regular platforms, we force their own ID
